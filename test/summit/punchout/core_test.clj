...
(defn ensure-broker-records []
  (let [broker (u/ensure-record :brokers {:name "Test Broker" :authkey "testing"})
        company (u/ensure-record :companies {:name "Test Company"})
        broker_company (u/ensure-record :broker_companies {:broker_id (:id broker) :company_id (:id company) :company_key "from-company" :company_auth_hash "super-secret"})
        ]
    broker_company))

(deftest extract-header-test
  (let [h      (extract-header-data (xml->map punchout-test-str))
        from   (:from h)
        to     (:to h)
        sender (:sender h)]
