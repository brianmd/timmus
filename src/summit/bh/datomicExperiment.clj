(ns summit.bh.datomicExperiment
...
;; (def uri "datomic:sql://testing?jdbc:mysql://localhost:3306/datomic?user=datomic_txr&password=qaDYAuNWHbU9oqN9DqaDYAuNWHbU9oqN9D")
;; (d/delete-database uri)
;; (d/create-database uri)         ;; see above command

(def conn (d/connect uri))

(def path "/Users/bmd/code/datomic-pro-0.9.5344/samples/seattle/")
...
(psorted (q '[:find ?n :where [?c :district/region ?n]] (db conn)))
(psorted (q '[:find ?n :where [?c :district/name ?n]] (db conn)))
(psorted (q '[:find ?n :where [?c :neighborhood/name ?n]] (db conn)))
(psorted (q '[:find ?n :where [?c :community/name ?n]] (db conn)))

(psorted (q '[:find ?n :where [?c :community/category ?n]] (db conn)))
;; (psorted (q '[:find (pull ?n [*]) :where [?c :community/type ?n]] (db conn)))
